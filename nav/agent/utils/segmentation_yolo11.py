"""
YOLOv11-seg instance segmentation backend for PEANUT.

Drop-in replacement for SemanticPredMaskRCNN / SemanticPredYOLO.
Requires ultralytics >= 8.3.0 (Python 3.8+).
...
class SemanticPredYOLO11:
    """
    YOLOv11-seg based instance segmentation for PEANUT ObjectNav.
    ...
    def __init__(self, args):
        ...
        # Model path: use yolo11_model_path if set, else fall back to seg_model_wts
        model_path = getattr(args, 'yolo11_model_path', None) or args.seg_model_wts
        ...
        print(f"[SemanticPredYOLO11] Model: {model_path}")
        print(f"[SemanticPredYOLO11] ultralytics YOLO11, {len(names)} classes")
        print(f"[SemanticPredYOLO11] YOLO->PEANUT map: {self.yolo_to_peanut}")
        print(f"[SemanticPredYOLO11] Device: {self.device}")

    def get_prediction(self, img, depth=None, goal_cat=None):
        """
        Run YOLOv11-seg on an RGB image and return per-category mask tensor.
        ...
        # Thresholds
        yolo_conf = getattr(args, 'yolo11_conf', 0.15)
        yolo_goal_conf = getattr(args, 'yolo11_goal_conf', 0.25)

        # Run inference — use a low pre-filter, post-filter per class
